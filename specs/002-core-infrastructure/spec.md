...
**Created**: 2026-04-07
**Status**: Implemented
**Input**: User description: "Self-contained .NET 10 solution scaffold for Relego: four projects (Core, Server, Cli, Tests), shared domain models (Highlight, Book, Author, User, Settings), SQLite schema creation on server startup, Serilog logging with file and SQLite sinks. No business logic — infrastructure only."
...
**Independent Test**: Start the server against an empty volume, inspect the database schema with `sqlite3 /data/relego.db .schema` — all tables must be present.

**Acceptance Scenarios**:

1. **Given** a Docker volume with no `relego.db` file, **When** the server starts, **Then** `relego.db` is created with all required tables
2. **Given** a `relego.db` that already has the correct schema, **When** the server restarts, **Then** the schema creation is skipped without error (idempotent)
3. **Given** a `relego.db` with a partial schema, **When** the server starts, **Then** missing tables are created without affecting existing ones
...
**Independent Test**: Start the server, trigger one HTTP request, verify that a log file exists under `/data/logs/` and that the `Logs` table in `relego.db` contains at least one entry.

**Acceptance Scenarios**:

1. **Given** the server is running, **When** any operation is executed, **Then** a log entry appears in the daily rolling log file under `/data/logs/relego-YYYYMMDD.log`
2. **Given** the server is running, **When** any operation is executed, **Then** a log entry appears in the `Logs` table in `relego.db`
...
- What happens when `relego.db` exists but is corrupted?
...
- **FR-001**: Solution MUST contain exactly four projects: `Relego.Core`, `Relego.Server`, `Relego.Cli`, `Relego.Tests`
- **FR-002**: `Relego.Core` MUST define shared domain models: `Highlight`, `Book`, `Author`, `User`, `Settings`
- **FR-003**: Server MUST create the SQLite schema on startup if it does not exist (idempotent — safe to run on every startup)
- **FR-004**: Server MUST configure Serilog with a rolling file sink (`/data/logs/relego-.log`, daily rotation) and a SQLite sink (`/data/relego.db`, `Logs` table)
- **FR-005**: All projects MUST target .NET 10
- **FR-006**: `Relego.Server` and `Relego.Cli` MUST reference `Relego.Core`
- **FR-007**: `Relego.Tests` MUST reference all other projects
...
- **User**: A Relego user. Key fields: id, kindle_email, created_at
...
- **SC-002**: Server startup creates `relego.db` with correct schema in under 5 seconds against an empty volume
- **SC-003**: After server start, at least one log entry is present in both `/data/logs/` and the `Logs` table in `relego.db`
...
- The `Relego.Cli` project scaffolded here contains no commands yet — that is feature 007
