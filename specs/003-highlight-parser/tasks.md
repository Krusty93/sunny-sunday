...
**Tests**: TDD approach — write tests before or alongside implementation. Tests go in `src/Relego.Tests/Parsing/ClippingsParserTests.cs`.
...
- [X] T001 Create directories `src/Relego.Cli/Parsing/` and `src/Relego.Tests/Parsing/`, verify `dotnet build src/Relego.slnx` still succeeds

---

## Phase 2: Foundational — Parser Data Types

**Purpose**: Define all parser-specific types that every user story depends on. These types live in `Relego.Cli/Parsing/` namespace (parser is CLI-exclusive, not shared with the server).

**⚠️ CRITICAL**: No user story work can begin until all types are defined and the solution builds.

- [X] T002 [P] Create `RawClipping` internal record in `src/Relego.Cli/Parsing/RawClipping.cs`
- [X] T003 [P] Create `ParsedHighlight` public record in `src/Relego.Cli/Parsing/ParsedHighlight.cs`
- [X] T004 [P] Create `ParsedBook` public record in `src/Relego.Cli/Parsing/ParsedBook.cs`
- [X] T005 [P] Create `ParseResult` public record in `src/Relego.Cli/Parsing/ParseResult.cs`
- [X] T006 Verify `dotnet build src/Relego.slnx` succeeds with all four new types. Fix any compilation errors.
...
- [X] T014 [US1] Create `ClippingsParser` static class in `src/Relego.Cli/Parsing/ClippingsParser.cs`
...
- [X] T020 [US1] Run `dotnet test src/Relego.Tests/Relego.Tests.csproj --filter "FullyQualifiedName~Parsing"` — all US1 tests must pass.
...
- [X] T021 [P] [US2] Write test in `src/Relego.Tests/Parsing/ClippingsParserTests.cs`: given input where the same highlight text appears twice for the same book and author, the result contains only one instance of that highlight and `DuplicatesRemoved == 1`.
...
- [X] T025 [US2] Implement deduplication in `src/Relego.Cli/Parsing/ClippingsParser.cs`: after building `RawClipping` list and filtering bookmarks, use a `HashSet<(string, string?, string)>` keyed on `(Title, Author, Text)` with `StringComparison.Ordinal` semantics to deduplicate. Keep the first occurrence (file order). Increment a duplicates counter. Wire `DuplicatesRemoved` into the returned `ParseResult`.
- [X] T026 [US2] Run `dotnet test src/Relego.Tests/Relego.Tests.csproj --filter "FullyQualifiedName~Parsing"` — all US1 and US2 tests must pass.
...
- [X] T027 [P] [US3] Write test in `src/Relego.Tests/Parsing/ClippingsParserTests.cs`: given 6 highlights across 3 books by different authors, `ParseResult.Books` contains exactly 3 `ParsedBook` entries, each with the correct title, author, and highlight count.
...
- [X] T030 [US3] Verify grouping logic in `src/Relego.Cli/Parsing/ClippingsParser.cs`: the grouping by `(Title, Author)` should already be implemented from T019. Ensure that: (a) books are emitted in first-seen order; (b) highlights within each book are in file order; (c) books with zero highlights after filtering are excluded. Adjust implementation if needed.
- [X] T031 [US3] Run `dotnet test src/Relego.Tests/Relego.Tests.csproj --filter "FullyQualifiedName~Parsing"` — all US1, US2, and US3 tests must pass.
...
- [X] T040 [US4] Run `dotnet test src/Relego.Tests/Relego.Tests.csproj --filter "FullyQualifiedName~Parsing"` — all US1, US2, US3, and US4 tests must pass.
...
- [X] T041 [P] Write edge case tests in `src/Relego.Tests/Parsing/ClippingsParserTests.cs`: (a) input with UTF-8 BOM prefix parses correctly (StreamReader handles BOM automatically, but verify via file-path overload with a BOM-prefixed temp file); (b) book titles and authors with Unicode characters (CJK, diacritics, RTL) parse correctly; (c) highlight text with special characters (quotes, angle brackets, ampersands) is preserved verbatim.
- [X] T042 [P] Write performance test in `src/Relego.Tests/Parsing/ClippingsParserTests.cs`: programmatically generate a `My Clippings.txt` string with 10,000 clipping entries, parse via `StringReader`, and assert completion within 5 seconds (SC-005). Use `[Fact]` with a `Stopwatch` or `[Fact(Timeout = 5000)]`.
- [X] T043 [P] Update `docs/ARCHITECTURE.md` to document the `Relego.Cli/Parsing/` component: its purpose (pure function parser for My Clippings.txt), public API surface (`ClippingsParser.ParseAsync`), key types (`ParseResult`, `ParsedBook`, `ParsedHighlight`), and design decisions (no dependencies, streaming, skip-and-log, notes as `[my note]`-prefixed highlights).
- [X] T044 Run full `dotnet test src/Relego.Tests/Relego.Tests.csproj` — all tests (including existing SchemaBootstrapTests) must pass. Run `dotnet build src/Relego.slnx` clean. Validate that the quickstart.md code examples in `specs/003-highlight-parser/quickstart.md` are consistent with the implemented API.
...
- All parser code uses `namespace Relego.Cli.Parsing;` file-scoped namespace
- All test code uses `namespace Relego.Tests.Parsing;` file-scoped namespace
...
- No changes to existing `Relego.Core/Models/` classes
