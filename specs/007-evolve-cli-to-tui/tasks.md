...
- [ ] T009 [P] [US8] Write `src/SunnySunday.Tests/Tui/BookGroupingTests.cs`: test `BookViewModel.FromHighlights()`: multiple highlights from same book → single `BookViewModel` with correct count, different books → separate entries, empty list → empty result, highlights with same title but different author → separate books.
...
- [ ] T014 [P] [US7] Create `src/SunnySunday.Cli/Tui/SearchFilter.cs`: static class with method `List<BookViewModel> Apply(IEnumerable<BookViewModel> books, string query)`. Filters books where title, author, or any highlight text contains the query (case-insensitive, `OrdinalIgnoreCase`). Returns matching books with all their highlights.
- [ ] T015 [US7] Integrate search into `BookListScreen`: when search is active, keystrokes append to `searchQuery`, `Esc` clears search, `Backspace` removes last character. After each keystroke, apply `SearchFilter.Apply()` to `filteredBooks` and reset `selectedIndex` to 0. `Render()` uses `filteredBooks` when search is active. Show search input line above table. Show "No results matching '[query]'" for empty results.
...
- [ ] T017 [P] [US7] Write `src/SunnySunday.Tests/Tui/SearchFilterTests.cs`: test match by title, match by author, match by highlight text, case-insensitive match, no match returns empty, empty query returns all, partial match works.
