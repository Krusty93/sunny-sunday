...
**Organization**: Tasks are grouped by user story so each slice stays independently testable after the shared foundation is in place. The plan defines 6 phases matching 7 user stories: Phase 1 covers US1 (Dual-Mode) + US7 (Render Loop) as foundational infrastructure, Phase 2 covers US2 (Chrome), Phase 3 covers US3 (Book List) + US6 (Search), Phase 4 covers US4 (Highlight Detail), Phase 5 covers US5 (Settings), and Phase 6 handles polish and edge cases.
...
**Purpose**: Implement the main landing screen showing all imported books in a navigable table, plus the client-side search filter. Covers US3 (Book List Main Screen) and US6 (Client-Side Search).
...
- [ ] T014 [P] [US6] Create `src/SunnySunday.Cli/Tui/SearchFilter.cs`: static class with method `List<BookViewModel> Apply(IEnumerable<BookViewModel> books, string query)`. Filters books where title, author, or any highlight text contains the query (case-insensitive, `OrdinalIgnoreCase`). Returns matching books with all their highlights.
- [ ] T015 [US6] Integrate search into `BookListScreen`: when search is active, keystrokes append to `searchQuery`, `Esc` clears search, `Backspace` removes last character. After each keystroke, apply `SearchFilter.Apply()` to `filteredBooks` and reset `selectedIndex` to 0. `Render()` uses `filteredBooks` when search is active. Show search input line above table. Show "No results matching '[query]'" for empty results.
- [ ] T016 [US3] Wire `BookListScreen` as the initial screen in `TuiApp.RunAsync()`: push `BookListScreen` onto the screen stack on startup.
- [ ] T017 [P] [US6] Write `src/SunnySunday.Tests/Tui/SearchFilterTests.cs`: test match by title, match by author, match by highlight text, case-insensitive match, no match returns empty, empty query returns all, partial match works.
- [ ] T018 [P] [US3] Write `src/SunnySunday.Tests/Tui/BookListScreenTests.cs`: test key handling: `↑/↓` changes `selectedIndex` within bounds, `Enter` returns `Push` with correct book, `Q` returns `Quit`, `/` activates search mode. Test empty state when no books loaded.
...
**Purpose**: Implement the settings page for editing Kindle email, recap schedule, highlight count, and sending a test email. Covers US5 (Settings Page).
...
- [ ] T022 [P] [US5] Write `src/SunnySunday.Tests/Tui/SettingsScreenTests.cs`: test key handling: navigation, `Esc` returns `Pop`, edit mode activation. Test validation: invalid email rejected, invalid count rejected, valid values accepted.
...
- [ ] T023 [P] Implement terminal size check in `src/SunnySunday.Cli/Tui/TuiApp.cs`: on each render cycle, check `Console.WindowWidth` and `Console.WindowHeight`. If below 80×24, render a centered message: "Terminal too small. Please resize to at least 80×24." instead of the normal layout.
- [ ] T024 [P] Implement graceful server disconnection handling in TUI screens: when any `SunnyHttpClient` call throws `HttpRequestException` during a TUI action, catch it, update `StatusChrome.IsConnected = false`, and display an inline error message in the content area (e.g., "Cannot reach server. Check connection."). Do not crash the TUI.
- [ ] T025 [P] Implement Figlet fallback in `src/SunnySunday.Cli/Tui/StatusChrome.cs`: if `Console.WindowWidth < 60`, render plain `Markup("[bold]SunnySunday[/]")` instead of `FigletText`.
- [ ] T026 Implement clean exit in `src/SunnySunday.Cli/Tui/TuiApp.cs`: on quit (`Q` or `Ctrl+C`), restore `Console.TreatControlCAsInput = false`, clear the live display, and return control to the terminal cleanly.
- [ ] T027 [P] Update `docs/ARCHITECTURE.md`: add TUI section describing dual-mode detection, render loop pattern, screen architecture, and data flow. Add Mermaid diagram showing TUI components and their relationship to the existing infrastructure.
- [ ] T028 Run full test suite: `dotnet test src/SunnySunday.slnx`. Confirm no regressions across all existing parser, API, CLI, infrastructure, and recap tests. All new TUI tests pass.
...
- **Phase 3 (Book List + Search)**: Depends on Phase 2 (chrome provides the layout context). **BLOCKS Phases 4 and 5** (they are navigated from the book list).
...
- **US7 (Render Loop)**: Phase 1 — foundational, blocks everything.
- **US2 (Chrome)**: Phase 2 — depends on render loop from Phase 1.
- **US3 (Book List)**: Phase 3 — depends on chrome from Phase 2.
- **US6 (Search)**: Phase 3 — co-located with book list, depends on chrome from Phase 2.
- **US4 (Highlight Detail)**: Phase 4 — depends on book list from Phase 3. Independent of US5.
- **US5 (Settings)**: Phase 5 — depends on book list from Phase 3. Independent of US4.
...
- Within Phase 6: `T023`, `T024`, `T025`, `T027` can run in parallel (different files/concerns).
...
```text
Phase 3 ──┬──► Phase 4 (T019, T020) ──┐
           └──► Phase 5 (T021, T022) ──┼──► Phase 6
```
...
4. Land Phase 4 (Highlight Detail) and Phase 5 (Settings) in parallel or either order.
5. Finish with Phase 6 (Polish) — edge cases, docs, full regression suite.
...
- No server-side changes — TUI uses existing REST API surface via `SunnyHttpClient`.
