...
**Testing**: xUnit + RichardSzalay.MockHttp (typed HttpClient mocking) — existing test project `Relego.Tests`
...
| III. Zero-Config Onboarding | **PASS** | Only `RELEGO_SERVER` env var required; Kindle path auto-detected |
...
**Post-design re-check**: All gates pass. New NuGet packages (`Spectre.Console.Cli`, `Microsoft.Extensions.Http`, `Microsoft.Extensions.DependencyInjection`, `RichardSzalay.MockHttp` for tests) align with technology constraints. No new projects introduced beyond the existing `Relego.Cli`.
...
├── Relego.Cli.csproj              ← Updated: new package references
├── Infrastructure/
...
├── Commands/
│   ├── SyncCommand.cs                  ← NEW: relego sync [path]
│   ├── StatusCommand.cs                ← NEW: relego status
│   ├── Config/
│   │   ├── ConfigShowCommand.cs        ← NEW: relego config show
│   │   ├── ConfigScheduleCommand.cs    ← NEW: relego config schedule <cadence> <time>
│   │   └── ConfigCountCommand.cs       ← NEW: relego config count <n>
│   ├── Exclude/
│   │   ├── ExcludeAddCommand.cs        ← NEW: relego exclude <type> <id>
│   │   ├── ExcludeRemoveCommand.cs     ← NEW: relego exclude remove <type> <id>
│   │   └── ExcludeListCommand.cs       ← NEW: relego exclude list
│   └── Weight/
│       ├── WeightSetCommand.cs         ← NEW: relego weight set <id> <value>
│       └── WeightListCommand.cs        ← NEW: relego weight list
...
**Structure Decision**: All CLI code lives in the existing `src/Relego.Cli` project. Commands are organized in a `Commands/` folder with subfolders for grouped verbs (`Config/`, `Exclude/`, `Weight/`). Infrastructure (HTTP client, DI bridge, Kindle detection) lives in `Infrastructure/`. No new .NET project is created.
...
- [ ] T000 Update `src/Relego.Cli/Relego.Cli.csproj`: add package references for `Spectre.Console.Cli` (0.55.0), `Microsoft.Extensions.Http` (10.0.6), `Microsoft.Extensions.DependencyInjection` (10.0.6), `Microsoft.Extensions.Hosting` (10.0.6). Add `RichardSzalay.MockHttp` (7.0.0) to `src/Relego.Tests/Relego.Tests.csproj`. Verify solution builds: `dotnet build src/Relego.slnx`.
- [ ] T001 Create `src/Relego.Cli/Infrastructure/TypeRegistrar.cs`: implement `ITypeRegistrar` and `ITypeResolver` bridging Spectre.Console.Cli to `Microsoft.Extensions.DependencyInjection.IServiceProvider`. The registrar wraps an `IServiceCollection`, builds the provider on first resolution. Namespace `Relego.Cli.Infrastructure`.
- [ ] T002 Create `src/Relego.Cli/Infrastructure/SunnyHttpClient.cs`: a typed HTTP client class with constructor taking `HttpClient`. Properties: none (base address set at registration). Methods:
  ...
  All methods throw `HttpRequestException` on non-success status codes (except 4xx which are returned for caller handling). Uses `System.Net.Http.Json` extension methods. Namespace `Relego.Cli.Infrastructure`.
- [ ] T003 Create `src/Relego.Cli/Infrastructure/HttpClientResilienceExtensions.cs`: static extension method `AddSunnyResilience(this IHttpClientBuilder builder)` that configures Polly retry pipeline: handle `HttpRequestException`, retry on status 408/429/5xx, max 3 attempts, exponential backoff (1s, 2s, 4s). Uses `Microsoft.Extensions.Http.Resilience` or raw Polly `ResiliencePipelineBuilder<HttpResponseMessage>`. Namespace `Relego.Cli.Infrastructure`.
- [ ] T004 Rewrite `src/Relego.Cli/Program.cs`:
  1. Read `RELEGO_SERVER` from environment; if missing or malformed URI, write Spectre error markup and `return 1`.
  2. Build `IServiceCollection`: register `SunnyHttpClient` as typed client with base address from `RELEGO_SERVER`, apply Polly retry via `AddSunnyResilience()`.
...
**Purpose**: Implement the Kindle auto-detection and the `relego sync` command — the most important user-facing feature.

- [ ] T005 Create `src/Relego.Cli/Infrastructure/KindleDetector.cs`. Static class with method `string? DetectClippingsPath()`:
...
- [ ] T006 Create `src/Relego.Cli/Commands/SyncCommand.cs`. Spectre.Console.Cli `AsyncCommand<SyncCommand.Settings>`:
...
- [ ] T007 Write `src/Relego.Tests/Cli/SyncCommandTests.cs`:
...
- [ ] T008 Write `src/Relego.Tests/Cli/KindleDetectorTests.cs`: test that the detector returns null when no Kindle paths exist (the default in CI). Platform-specific tests can use a temp directory structure to simulate the expected path layout.

**Checkpoint**: `relego sync /path/to/file` works end-to-end against a mock server. `dotnet test --filter "FullyQualifiedName~Cli.Sync"` passes.

---

## Phase 3: Status Command (US-7)

**Purpose**: Implement `relego status` to display server health and state.

- [ ] T009 Create `src/Relego.Cli/Commands/StatusCommand.cs`. Spectre.Console.Cli `AsyncCommand` (no settings needed):
  - `ExecuteAsync`:
    ...
    3. Render table.
    4. Return 0.
  - On connection error: display actionable error with `RELEGO_SERVER` value and suggestion to check the variable.
  Register as `app.Configure(c => c.AddCommand<StatusCommand>("status"))`.
- [ ] T010 Write `src/Relego.Tests/Cli/StatusCommandTests.cs`:
...
- [ ] T011 Create `src/Relego.Cli/Commands/Config/ConfigShowCommand.cs`. `AsyncCommand` registered as `relego config show`:
  - Fetch `GET /settings`.
  - Display table: Schedule, Delivery Day (if weekly), Delivery Time (local), Count, Kindle Email, Timezone.
- [ ] T012 Create `src/Relego.Cli/Commands/Config/ConfigScheduleCommand.cs`. `AsyncCommand<ConfigScheduleCommand.Settings>`:
...
- [ ] T013 Create `src/Relego.Cli/Commands/Config/ConfigCountCommand.cs`. `AsyncCommand<ConfigCountCommand.Settings>`:
...
- [ ] T015 Write `src/Relego.Tests/Cli/ConfigCommandTests.cs`:
...
- [ ] T016 Create `src/Relego.Cli/Commands/Exclude/ExcludeAddCommand.cs`. `AsyncCommand<ExcludeAddCommand.Settings>`:
...
- [ ] T017 Create `src/Relego.Cli/Commands/Exclude/ExcludeRemoveCommand.cs`. `AsyncCommand<ExcludeRemoveCommand.Settings>`:
...
- [ ] T018 Create `src/Relego.Cli/Commands/Exclude/ExcludeListCommand.cs`. `AsyncCommand`:
...
- [ ] T020 Write `src/Relego.Tests/Cli/ExcludeCommandTests.cs`:
...
- [ ] T021 Create `src/Relego.Cli/Commands/Weight/WeightSetCommand.cs`. `AsyncCommand<WeightSetCommand.Settings>`:
  ...
  - Display confirmation.
- [ ] T022 Create `src/Relego.Cli/Commands/Weight/WeightListCommand.cs`. `AsyncCommand`:
...
- [ ] T024 Write `src/Relego.Tests/Cli/WeightCommandTests.cs`:
...
- [ ] T025 Ensure all commands handle HTTP errors uniformly: connection refused → "Cannot reach server at {url}. Check RELEGO_SERVER environment variable."; 4xx → parse error body and display field messages; 5xx after retries → "Server error. Try again later."
...
- [ ] T028 Run full test suite: `dotnet test src/Relego.slnx`. Confirm no regressions. All new CLI tests pass alongside existing parser, infrastructure, API, and recap tests.
