...
**Created**: 2026-05-02  
**Status**: Draft  
**Input**: User request: "Implement the relego client CLI binary (Spectre.Console) with all commands: upload highlights, configure server, manage weights, exclusions, and recaps."

## User Scenarios & Testing

### User Story 1 - Sync Highlights to Server (Priority: P1)

A user runs `relego sync` to parse their Kindle clippings file and upload all highlights to the server in a single command.
...
**Independent Test**: Run `relego sync /path/to/My Clippings.txt` against a running server and verify highlights appear in the server database.

**Acceptance Scenarios**:

1. **Given** a valid `My Clippings.txt` file path, **When** the user runs `relego sync /path/to/file`, **Then** highlights are parsed and POSTed to the server via `/sync`.
2. **Given** a connected Kindle at a known mount path, **When** the user runs `relego sync` without arguments, **Then** the CLI auto-detects the clippings file and syncs.
3. **Given** no path argument and no Kindle detected, **When** the user runs `relego sync`, **Then** the CLI prompts the user to provide a file path.
...
5. **Given** the server is unreachable, **When** the user runs `relego sync`, **Then** the CLI displays an actionable error message with the server URL attempted.

---

### User Story 2 - Configure Schedule (Priority: P1)

A user configures when recap emails are delivered using `relego config schedule`.

**Why this priority**: Schedule configuration is required for the core value loop — unattended recap delivery.

**Independent Test**: Run `relego config schedule daily 08:00` and verify the server settings reflect the new schedule and timezone.

**Acceptance Scenarios**:

1. **Given** valid cadence and time, **When** the user runs `relego config schedule daily 18:00`, **Then** the CLI sends PUT /settings with cadence, time, and detected timezone.
2. **Given** the user wants to see current schedule, **When** the user runs `relego config schedule show`, **Then** the CLI fetches and displays the current schedule in local time.
3. **Given** an invalid time format, **When** the user enters `relego config schedule daily 25:00`, **Then** the CLI shows a validation error without contacting the server.
...
**Independent Test**: Run `relego config count 10` and verify the server settings are updated.

**Acceptance Scenarios**:

1. **Given** a valid count (1–15), **When** the user runs `relego config count 10`, **Then** the CLI sends PUT /settings with the new count.
2. **Given** the user wants to see current count, **When** the user runs `relego config count show`, **Then** the CLI displays the current highlight count.
3. **Given** an out-of-range count, **When** the user runs `relego config count 0` or `relego config count 20`, **Then** the CLI shows a validation error.

---

### User Story 4 - View All Settings (Priority: P2)

A user views all current configuration in one place using `relego config show`.

**Why this priority**: Users need a quick overview without running multiple commands.

**Independent Test**: Run `relego config show` and verify a table displays schedule, count, and Kindle email.

**Acceptance Scenarios**:

1. **Given** a configured server, **When** the user runs `relego config show`, **Then** the CLI displays a table with all current settings (schedule cadence, time, count, Kindle email).
...
**Independent Test**: Run `relego exclude book "Some Book"` and verify the book no longer appears in recap selection.

**Acceptance Scenarios**:

1. **Given** a valid highlight/book/author identifier, **When** the user runs `relego exclude highlight|book|author <id|name>`, **Then** the CLI sends POST to the appropriate exclusion endpoint.
2. **Given** an existing exclusion, **When** the user runs `relego exclude remove highlight|book|author <id|name>`, **Then** the CLI sends DELETE to remove the exclusion.
3. **Given** existing exclusions, **When** the user runs `relego exclude list`, **Then** the CLI displays all exclusions in a formatted table grouped by type.
...
**Independent Test**: Run `relego weight set <id> 5` and verify the weight is updated on the server.

**Acceptance Scenarios**:

1. **Given** a valid highlight ID and weight value, **When** the user runs `relego weight set <id> <value>`, **Then** the CLI sends PUT /highlights/{id}/weight with the new value.
2. **Given** highlights with custom weights, **When** the user runs `relego weight list`, **Then** the CLI displays a table of highlights with their current weights.
...
A user checks the health and state of the connected server with `relego status`.

**Why this priority**: Users need to verify connectivity and see at-a-glance system state.

**Independent Test**: Run `relego status` against a running server and verify the output shows total highlights, last sync, next recap, and server version.

**Acceptance Scenarios**:

1. **Given** a reachable server, **When** the user runs `relego status`, **Then** the CLI displays total highlights, last sync time, next scheduled recap, delivery status, and server version.
2. **Given** server timestamps are UTC, **When** status is displayed, **Then** times are converted to local timezone.
3. **Given** an unreachable server, **When** the user runs `relego status`, **Then** the CLI displays a clear connection error with the URL attempted and suggests checking RELEGO_SERVER.

---

### User Story 8 - Server URL Configuration (Priority: P1)

A user sets the server URL via the `RELEGO_SERVER` environment variable. The CLI resolves where to connect on every invocation.

**Why this priority**: Without server connectivity, no CLI command can function.

**Independent Test**: Set `RELEGO_SERVER=http://localhost:5000` and run any command; verify it connects to that address.

**Acceptance Scenarios**:

1. **Given** `RELEGO_SERVER` is set, **When** any command runs, **Then** the CLI uses that URL as the base address for all API calls.
2. **Given** `RELEGO_SERVER` is not set, **When** any command runs, **Then** the CLI displays an error explaining how to set the variable.
3. **Given** `RELEGO_SERVER` is set to a malformed URL, **When** any command runs, **Then** the CLI displays a validation error.

---

### Edge Cases

- **Empty clippings file**: `relego sync` with a valid but empty file displays "No highlights found" and exits cleanly.
...
- **FR-006-02**: CLI MUST resolve server URL from the `RELEGO_SERVER` environment variable on every invocation.
- **FR-006-03**: CLI MUST display an actionable error if `RELEGO_SERVER` is not set or is malformed.
...
- **FR-006-12**: CLI MUST use `relego config show` to display all current settings in a unified table.
...
- **FR-006-15**: CLI MUST support `relego weight set <id> <value>` to update highlight weights.
- **FR-006-16**: CLI MUST support `relego weight list` to display highlights with their weights.
- **FR-006-17**: CLI MUST use `relego status` to display server health, totals, last sync, next recap, and version.
...
- **SC-006-01**: Users can sync highlights from a Kindle clippings file in a single command (`relego sync`) with zero manual configuration beyond `RELEGO_SERVER`.
...
- The existing ClippingsParser in `Relego.Cli/Parsing/` is correct and complete for MVP clippings format.
- Single-user architecture (user id 1) — no user switching or multi-tenant concerns.
- `RELEGO_SERVER` includes the scheme and port (e.g., `http://localhost:5000`) — no default port inference.
