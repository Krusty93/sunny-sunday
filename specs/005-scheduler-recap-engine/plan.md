...
- [ ] T008 Create `src/SunnySunday.Server/Infrastructure/Smtp/SmtpSettings.cs` with properties: `Host` (string, default `"smtp.gmail.com"`), `Port` (int, default `587`), `Username` (string, empty), `Password` (string, empty), `UseSsl` (bool, default `true`). Namespace `SunnySunday.Server.Infrastructure.Smtp`. Add configuration mapping so env vars (`SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD`, `SMTP_USE_SSL`) populate this options object.
